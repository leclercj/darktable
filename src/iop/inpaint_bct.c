...
static void SmoothImage(Data *data);
static void OrderByDistance(Data *data);
static void InitTfieldAndHeap(Data *data, Heap *H);
static void TfieldDefaultInitialization(Data *data);
static int TfieldAdaptInitializationToImage(Data *data);
static double solve(Data *data, int i, int j);
static void InpaintByOrder(Data *data);
static void SmoothUpdate(Data *data,int xi, int xj);
static void inpaintPoint(Data *data,int i, int j);
static void Guidance(Data *data, int xi, int xj, double *G);
static void ModStructureTensor(Data *data, int xi, int xj, double *st);
static double euclidean_norm(double *v);



static Heap *Heap_new(Data *data)
{
...
static Heap *Heap_free(Heap *heap)
{
	free(heap);
...
static void Heap_upHeap(Heap *heap, int pos)
{
	int help = pos;
	hItem *h;
	...
	while ( (help != 0) && (heap->heap[(help-1)/2]->T > heap->heap[pos]->T))
	{
		help = (help-1)/2;
		...
		if (help != pos)
		{
			// swap
			h = heap->heap[help];
			heap->heap[help] = heap->heap[pos];
			heap->heap[help]->hpos = help;
			heap->heap[pos] = h;
			heap->heap[pos]->hpos = pos;
		}
...
static void Heap_downHeap(Heap *heap, int pos)
{
	int help = 2*pos + 1; // left child
	hItem *h;

	if (help > heap->size) // no children
		return;
	
	// compare left with right child, that is help+1, if it exists
	if ( (help +1 <= heap->size) && (heap->heap[help]->T > heap->heap[help+1]->T))
		help++;
	// help is now index with Tmin

	while( (help <= heap->size)  && (heap->heap[help]->T < heap->heap[pos]->T))
	{
		// swap
		h = heap->heap[help];
		heap->heap[help] = heap->heap[pos];
...
static int Heap_isempty(Heap *heap)
{
	if( heap->size == -1)
		return 1;
	else
...
static void Heap_insert(Heap *heap, hItem item)
{
  int index;
  
  index = item.j * heap->pdata->rows + item.i;
	...
	if(item.flag == BAND)
	{
		if(item.hpos == -1) // not yet in heap
		{
			...
			heap->pdata->Tfield[index].T = item.T;
			heap->size += 1;
			heap->pdata->Tfield[index].hpos = heap->size;
			heap->heap[heap->size] = &(heap->pdata->Tfield[index]);
			Heap_upHeap(heap, heap->size);

		}
		else // already in heap
		{
			// only T update
			double oldT = heap->heap[item.hpos]->T;
			heap->heap[item.hpos]->T = item.T;

			if(oldT > item.T)
				Heap_upHeap(heap, item.hpos);
			else
				Heap_downHeap(heap, item.hpos);
		}
	}
}

static hItem Heap_extract(Heap *heap)
{
	hItem ret;

	//heap->heap[0]->flag = KNOWN;
	ret = *heap->heap[0];
...
static void inpaintPoint(Data *data, int xi, int xj)
{
  int indexx;
  int indexy;
	int yi, yj;
  ...
  if (data->guidance == 2)
  {
    ...
    // debug
		// g_printf(" Wk is %lf , W is %lf , at %d %d \n",Wk,W,xi,xj );
	}
...
static void OrderByDistance(Data *data)
{
  ...
  InitTfieldAndHeap(data, NarrowBand);
	...
	while(!Heap_isempty(NarrowBand))
	{
		actual = Heap_extract(NarrowBand);
		...
		for(k=0 ; k<4; k++)
		{
			if(nbh[k] != NULL)
			{
				...
				if(nbh[k]->flag == BAND)
				{
					...
					Heap_insert(NarrowBand, temp);
				}
			}
		}
	}

  Heap_free(NarrowBand);
}
...
static void InitTfieldAndHeap(Data *data, Heap *H)
{
  ...
  if (err)
  {
    ...
    g_printf("Threshold aplha too large. Falling back to euclidean distance. \n");
  }
